...
class EDF_Hybrid
{
	...
	public static boolean decidePreemption(double current_time, PriorityQueue<Task> min_heap, List<Task> list_of_tasks)
	{
		if(min_heap.isEmpty() == false)
		{
			PriorityQueue<Task> min_heap1 = new PriorityQueue<Task>(new TaskComparator());

			for(Task t: min_heap)
				min_heap1.add(t); 

			Task current_task = min_heap1.poll();
			double if_executed_current = current_time + current_task.remaining_time;

			PriorityQueue<Task> task_set = new PriorityQueue<Task>(new TaskComparator());
			
			for(Task t:min_heap1)
				task_set.add(t);

			for(Task t:list_of_tasks)
			{
				if(t.arrival_time > current_time && t.arrival_time <= if_executed_current)
					task_set.add(t);
			}

			if(task_set.isEmpty() == false)
			{
				Task closest_task = task_set.poll();

				double preemption_factor = closest_task.scheduling_parameter - if_executed_current;

				if(preemption_factor >= closest_task.remaining_time)
					return true;
				else return false;
			}
		}
	...
	public static double edfhyb(List<Task> list_of_tasks)
	{
		...
		while(current_time<(2*limit))
		{
			...
			boolean isPreempt = decidePreemption(current_time, min_heap, list_of_tasks);
	...
	public static void main(String args[]) throws IOException
	{
		...
		Task t2 = new Task("t2", 0, 0, 2, 4);
		Task t3 = new Task("t3", 0, 1, 1, 2);
		...
		for(int i=0; i<prior.length; i++)
		{
			...
			list_of_tasks.add(new Task(name, prior[i], arriv[i], exect[i], deadl[i] + 1));
		}

		System.out.println("Time Taken Overall = \t\t"+edfhyb(list_of_tasks));
